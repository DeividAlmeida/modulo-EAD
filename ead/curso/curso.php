...
<script>
    ...
    new Vue({
        ...
        data: {
            ...
            config,
            categoria:[],
            categorias: [<?php if(is_array($categorias)){foreach($categorias as $nome){ echo '"'.$nome['nome'].'",';}} ?>],
            exibe: ["Sim", "Não"],
            professor:[],
            professores:[<?php if(is_array($professores)){foreach($professores as $nome){ echo '"'.$nome['nome'].'",';} }?>],
            preview:  [],
