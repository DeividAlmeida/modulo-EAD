...
<div class="card" >
    ...
    <div class="card-body" v-else>
        <form method="post" :action="'?aula='+status" enctype="multipart/form-data">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input  type='file' multiple class='form-control' accept=".zip,.rar,.7zip,video/*,image/*,audio/*,.doc,.txt,.odt,.odf,.docx,.pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"  name='arquivo' > 
                            <small v-if='idx.tipo == "local"'>{{idx.arquivo}}</small>
                    </div>
                </div>
            </div>
            <div class="row"  >
            ...
            <div v-for="ask, index of questions" >
                <div class="row"  >
                    <div class="col-md-11" >
                        <div class="form-group" >
                            <input    v-model="questions[index]" class='form-control'  name='questoes[]' :key="index">
                        </div>
                    </div>
                ...
                <div class="row justify-content-md-center" v-for="alter, ix of alternatives[index]" >
                     <div class="col col-md-1">
                        <div class="form-group">
                            <label>Resposta: </label>
                            <input class='form-control' :name='"gabarito["+index+"]"' v-model='idx.gabarito[index]' :value="ix" :id="index+ix"   type="radio"> 
                        </div>
                    </div>  
                    <div class="col col-md-6">
                        <div class="form-group">
                            <label>Alternativa: </label>
                            <input class='form-control' :name='"alternativas["+index+"]["+ix+"]"' v-model="alternatives[index][ix]"   :key="ix"> 
                        </div>
                    </div>  
                </div>
            </div>
    ...
    new Vue({
        ...
        methods:{
            move: function(a, b){
                this.status = a
                this.idx = b
                this.questions = JSON.parse(this.idx.questoes)
                this.alternatives = JSON.parse(this.idx.alternativas)
