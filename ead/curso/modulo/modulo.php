...
if($status > 0):$default = json_encode(DBRead("ead_modulo","*","WHERE id = '{$status}'")[0]); else: $default = '{"nome":"","cpf":"","endereco":"", "data":"" }'; endif;
$query = json_encode(DBRead('ead_modulo','*'));
?>
<div class="card"  >
    <div id="control" v-if="!status">
        ...
        <div class="card-body" v-else>
            <?php if (checkPermission($PERMISSION, $_SERVER['SCRIPT_NAME'], 'item', 'adicionar')) { ?>
                <div class="alert alert-info">Nenhum módulo adicionado a essa listagem até o momento, <a class="adicionarListagemItem" href="?routeModu=0" >clique aqui</a> para adicionar.</div>
            <?php } ?>
        </div>
    </div>
    <div class="card-body" v-else>
